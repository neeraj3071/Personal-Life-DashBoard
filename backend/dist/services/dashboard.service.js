...
class DashboardService {
    ...
    async updateSettings(userId, input) {
        const settings = await prisma_1.default.analyticsSettings.upsert({
            where: { userId },
            update: {
    ...
    createEmptyAggregate(date, habitCount) {
        return {
            date,
    ...
    async buildDailyAggregates(userId, startDate, endDate) {
        const rangeStart = (0, date_fns_1.startOfDay)(startDate);
        const rangeEnd = (0, date_fns_1.endOfDay)(endDate);
        const [sleepLogs, workoutLogs, moodLogs, expenses, habitLogs, habitCount] = await Promise.all([
            prisma_1.default.sleepLog.findMany({
                where: {
                    userId,
            ...
            prisma_1.default.workoutLog.findMany({
                where: {
                    userId,
                    date: { gte: rangeStart, lte: rangeEnd }
                },
            ...
            prisma_1.default.moodLog.findMany({
                where: {
                    userId,
                    date: { gte: rangeStart, lte: rangeEnd }
                },
            ...
            prisma_1.default.habitLog.findMany({
                where: {
                    date: { gte: rangeStart, lte: rangeEnd },
                    habit: { userId }
                },
            ...
            prisma_1.default.habit.count({ where: { userId } })
        ]);
        const dayMap = new Map();
        for (const day of (0, date_fns_1.eachDayOfInterval)({ start: rangeStart, end: rangeEnd })) {
        ...
        for (const sleepLog of sleepLogs) {
            ...
            let durationHours = (new Date(sleepLog.wakeTime).getTime() - new Date(sleepLog.sleepTime).getTime()) / (1000 * 60 * 60);
            if (durationHours < 0) {
                durationHours += 24;
            }
    ...
    calculateDailyLifeScore(aggregate, targets) {
        ...
        const score = Math.round(components.reduce((sum, component) => sum + component.score * component.weight, 0));
    ...
    calculateWeekMetrics(aggregates, raw, habitCount, startDate, endDate, totalDays) {
    ...
    getAtLeastStatus(current, target) {
        if (target <= 0) {
            return 'completed';
        }
    ...
    getAtMostStatus(current, target, elapsedDays) {
        if (target <= 0) {
            return 'completed';
        }
        if (elapsedDays >= 7) {
        ...
        if (current <= expectedSpend)
            return 'on-track';
        if (current <= target)
            return 'at-risk';
        return 'behind';
    }
    buildMission(id, title, description, direction, current, target, unit, rewardXp, elapsedDays) {
    ...
    getConfidenceBySampleSize(sampleSize) {
        if (sampleSize >= 16)
            return 'high';
        if (sampleSize >= 8)
            return 'medium';
    ...
    buildCorrelationInsights(aggregates, targets) {
        const insights = [];
        ...
        if (lowSleepDays.length >= 2 && healthySleepDays.length >= 2) {
            ...
            if (lowSleepMood !== null && healthySleepMood !== null) {
                ...
                const spendingDeltaPercent = healthySleepSpend > 0
                    ...
                    : 0;
                const impact = moodDrop > 0 || spendingDeltaPercent > 0 ? 'negative' : 'neutral';
        ...
        if (activeWorkoutDays.length >= 2 && inactiveWorkoutDays.length >= 2) {
            const activeMood = average(activeWorkoutDays.map((aggregate) => aggregate.mood)) ?? 0;
            const inactiveMood = average(inactiveWorkoutDays.map((aggregate) => aggregate.mood)) ?? 0;
    ...
    calculateConsistencyStreak(aggregates) {
        let streak = 0;
    ...
    calculateWeeklyMissionStreak(aggregates, raw, habitCount, targets, currentWeekStart) {
        let streakWeeks = 0;
        for (let offset = 1; offset <= 8; offset += 1) {
            const weekStart = (0, date_fns_1.startOfWeek)((0, date_fns_1.subWeeks)(currentWeekStart, offset), { weekStartsOn: WEEK_STARTS_ON });
            const weekEnd = (0, date_fns_1.endOfWeek)(weekStart, { weekStartsOn: WEEK_STARTS_ON });
            ...
            const habitCompletion = possibleHabitLogs > 0 ? (completedHabitLogs / possibleHabitLogs) * 100 : 0;
            const weekPasses = (avgSleep ?? 0) >= targets.sleepHours &&
    ...
    buildSmartNudges(weekMetrics, targets, elapsedDays, todayAggregate, hasYesterdaySleep) {
        const nudges = [];
    ...
    buildRuleBasedForecast(aggregates, targets) {
        ...
        const confidence = activeDays >= 6 ? 'high' : activeDays >= 4 ? 'medium' : 'low';
        const componentAverages = {
            sleep: average(aggregates
                .map((aggregate) => this.calculateDailyLifeScore(aggregate, targets).components.find((component) => component.key === 'sleep')?.score ?? 0)) ?? 0,
            workout: average(aggregates
                .map((aggregate) => this.calculateDailyLifeScore(aggregate, targets).components.find((component) => component.key === 'workout')?.score ?? 0)) ?? 0,
            habit: average(aggregates
                .map((aggregate) => this.calculateDailyLifeScore(aggregate, targets).components.find((component) => component.key === 'habit')?.score ?? 0)) ?? 0,
            mood: average(aggregates
                .map((aggregate) => this.calculateDailyLifeScore(aggregate, targets).components.find((component) => component.key === 'mood')?.score ?? 0)) ?? 0,
            spending: average(aggregates
                .map((aggregate) => this.calculateDailyLifeScore(aggregate, targets).components.find((component) => component.key === 'spending')?.score ?? 0)) ?? 0
        };
    ...
    async buildForecast(aggregates, targets, options) {
        const baseForecast = this.buildRuleBasedForecast(aggregates, targets);
        const recentSummary = {
            avgSleep: average(aggregates
                .map((aggregate) => aggregate.sleepHours)
                .filter((value) => value !== null)),
            avgMood: average(aggregates
                .map((aggregate) => aggregate.mood)
                .filter((value) => value !== null)),
            ...
            avgHabitCompletion: round1(average(aggregates.map((aggregate) => {
                if (aggregate.totalHabits === 0) {
                    return 0;
                }
        ...
        const enhancedForecast = await prediction_service_1.default.generateForecast({
            targets,
            baseForecast,
            recentSummary,
    ...
    buildTimelineEvents(raw, targets) {
        const events = [];
        ...
        for (const workoutLog of raw.workoutLogs) {
            const timestamp = (0, date_fns_1.startOfDay)(workoutLog.date);
        ...
        for (const habitLog of raw.habitLogs) {
            const timestamp = (0, date_fns_1.startOfDay)(habitLog.date);
        ...
        for (const moodLog of raw.moodLogs) {
            const timestamp = (0, date_fns_1.startOfDay)(moodLog.date);
        ...
        for (const expense of raw.expenses) {
            const timestamp = (0, date_fns_1.startOfDay)(expense.date);
    ...
    async getDashboardStats(userId) {
        ...
        const todayAggregate = bundle.aggregates.find((aggregate) => aggregate.dateKey === todayKey) ?? this.createEmptyAggregate(today, bundle.habitCount);
        const weekMetrics = this.calculateWeekMetrics(bundle.aggregates, bundle.raw, bundle.habitCount, weekStart, weekEnd, 7);
        const productivityScore = Math.round((weekMetrics.habitCompletion + clamp((weekMetrics.workoutSessions / 7) * 100, 0, 100)) / 2);
        return {
    ...
    async getPerformance(userId) {
        ...
        const settings = await this.getOrCreateSettingsRecord(userId);
        const targets = this.mapSettingsToTargets(settings);
        const rangeStart = (0, date_fns_1.startOfDay)((0, date_fns_1.subDays)(today, 60));
        const bundle = await this.buildDailyAggregates(userId, rangeStart, weekEnd);
        const last30Start = (0, date_fns_1.startOfDay)((0, date_fns_1.subDays)(today, 29));
        const last7Start = (0, date_fns_1.startOfDay)((0, date_fns_1.subDays)(today, 6));
        ...
        const elapsedDays = Math.min(7, (0, date_fns_1.differenceInCalendarDays)(today, weekStart) + 1);
        const weekMetrics = this.calculateWeekMetrics(weekAggregates, bundle.raw, bundle.habitCount, weekStart, weekEnd, elapsedDays);
        const missions = [
            this.buildMission('sleep-target', `Sleep ${formatTargetLabel(targets.sleepHours)}h+ Average`, 'Maintain your sleep recovery target all week.', 'at-least', weekMetrics.avgSleep ?? 0, targets.sleepHours, 'hrs avg', 120, elapsedDays),
            this.buildMission('workout-target', `${formatTargetLabel(targets.workoutSessions)} Workouts / Week`, 'Hit minimum training frequency this week.', 'at-least', weekMetrics.workoutSessions, targets.workoutSessions, 'sessions', 140, elapsedDays),
            this.buildMission('spending-target', 'Spend Under Weekly Cap', 'Stay below your defined weekly spending budget.', 'at-most', weekMetrics.spending, targets.weeklySpending, 'USD', 100, elapsedDays),
            this.buildMission('habit-target', `Habit Completion ${formatTargetLabel(targets.habitCompletion)}%+`, 'Keep completion consistency across active habits.', 'at-least', weekMetrics.habitCompletion, targets.habitCompletion, '%', 130, elapsedDays),
            this.buildMission('mood-target', `Average Mood ${formatTargetLabel(targets.mood)}+`, 'Protect emotional baseline with active recovery.', 'at-least', weekMetrics.avgMood ?? 0, targets.mood, '/5', 110, elapsedDays)
        ];
        const weeklyStreak = this.calculateWeeklyMissionStreak(bundle.aggregates, bundle.raw, bundle.habitCount, targets, weekStart);
        const streakRewards = {
        ...
        const [sleepCount, workoutCount, completedHabitCount, moodCount, expenseCount] = await Promise.all([
            prisma_1.default.sleepLog.count({ where: { userId } }),
            prisma_1.default.workoutLog.count({ where: { userId } }),
            prisma_1.default.habitLog.count({
                where: {
                    completed: true,
                    habit: { userId }
                }
            }),
            prisma_1.default.moodLog.count({ where: { userId } }),
            prisma_1.default.expense.count({ where: { userId } })
        ]);
        ...
        const levelProgressPercent = Math.round(clamp(((totalXp - currentLevelFloor) / Math.max(nextLevelXp - currentLevelFloor, 1)) * 100, 0, 100));
        const badges = [
            {
                id: 'sleep-sentinel',
                name: 'Sleep Sentinel',
        ...
        const nudges = this.buildSmartNudges(weekMetrics, targets, elapsedDays, todayAggregate, hasYesterdaySleep);
    ...
    async getTimeline(userId, range = 'week', dateInput) {
        const parsedDate = dateInput ? (0, date_fns_1.parseISO)(dateInput) : new Date();
        const referenceDate = (0, date_fns_1.isValid)(parsedDate) ? parsedDate : new Date();
        const settings = await this.getOrCreateSettingsRecord(userId);
        const targets = this.mapSettingsToTargets(settings);
        const periodStart = range === 'day'
            ? (0, date_fns_1.startOfDay)(referenceDate)
            : (0, date_fns_1.startOfWeek)(referenceDate, { weekStartsOn: WEEK_STARTS_ON });
        const periodEnd = range === 'day'
            ? (0, date_fns_1.endOfDay)(referenceDate)
            : (0, date_fns_1.endOfWeek)(referenceDate, { weekStartsOn: WEEK_STARTS_ON });
        ...
        const daySummaries = bundle.aggregates.map((aggregate) => {
            const lifeScore = this.calculateDailyLifeScore(aggregate, targets);
            return {
                date: aggregate.dateKey,
                label: (0, date_fns_1.format)(aggregate.date, 'EEE dd'),
